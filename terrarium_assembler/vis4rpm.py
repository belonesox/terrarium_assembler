...
def load_packages_from_container_image(image):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        subprocess.run(['podman', 'run', '--rm', '-u', '0', '-v', tmp+':/workdir:z', '-v', 'copy.sh:/copy.sh:z', image, '/bin/sh', '-c', cmd])
...
def compute_graph(packages, groups=None, closure=True):

    graph = {}

    limit_to_packages = set(packages.keys())
    for _, package in packages.items():
        ...
        if groups:
            for group in groups:
                if name in group["packages"]:
                    ...
                    node["type"] = "group"
                    node["dependencies"] = sorted(set(group["requires_resolved"]) & limit_to_packages)
                    node["weak_dependencies"] = list((set(group["recommends_resolved"]) | set(group["suggests_resolved"])) & limit_to_packages)
        ...
        if not node:
            ...
            if groups:
                pkg_deps_requires = set(package["requires_resolved"]) & limit_to_packages
                pkg_deps_recommends = set(package["recommends_resolved"]) & limit_to_packages
                pkg_deps_suggests = set(package["suggests_resolved"]) & limit_to_packages
            ...
            # Package -> Package relations
                ...
                node["dependencies"] = set(package["requires_resolved"]) & limit_to_packages 
                node["weak_dependencies"] = list((set(package["recommends_resolved"]) | set(package["suggests_resolved"])) & limit_to_packages)
